...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ open, onClose, onOpenAdmin }) => {
  ...
  const sendTip = async () => {
    ...
    setTipBusy(false);
    setTipMsg(error ? '❌ ব্যর্থ: ' + error.message : `🤖⚡ ধন্যবাদ! রোবট ৳${toBanglaNum(tip)} energy পেয়ে লাফ দিচ্ছে!`);
    setTipTrx('');
  };
  ...
  return (
    <div className="fixed inset-0 z-[70]">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-sm bg-white dark:bg-[#151b27] shadow-2xl overflow-y-auto p-5 text-slate-900 dark:text-slate-100">
        ...
        {/* ===== Donation — Gamified Robot  ===== */}
        {donationOn && (
          <div className="rounded-2xl bg-gradient-to-br from-violet-500/15 to-cyan-500/10 border border-violet-500/30 p-4 mb-3">
            <div className="flex items-center gap-2.5 mb-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-violet-500 to-cyan-500 flex items-center justify-center shadow-lg shadow-violet-500/30">
                <Bot className="w-5 h-5 text-white" />
              </div>
              <div>
                <div className="text-xs font-black text-white">ডেভেলপার রোবট 🤖</div>
                <div className="text-[10px] text-slate-400">ঐচ্ছিক tip — প্রিমিয়াম না, রোবটকে energy দাও ⚡</div>
              </div>
            </div>

            {/* Energy level bar (gamified) */}
            <div className="mb-3">
              <div className="flex items-center justify-between text-[9px] font-bold text-slate-400 mb-1">
                <span>রোবট energy লেভেল</span>
                <span className="text-cyan-300">⚡ {toBanglaNum(tip)}%</span>
              </div>
            ...
            <div className="flex items-center justify-between gap-2 mb-2 rounded-xl bg-[#0f141d] border border-white/10 px-3 py-2">
              ...
              <button onClick={copyNum} className="flex items-center gap-1 text-[10px] font-black text-pink-300 cursor-pointer">
                {numCopied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />} {BKASH_NUMBER}
              </button>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={sendTip} disabled={tipBusy}
                className="px-4 rounded-xl bg-gradient-to-r from-violet-500 to-cyan-500 text-white text-[11px] font-black cursor-pointer disabled:opacity-50 shadow-lg shadow-violet-500/25">
                পাঠাও
              </button>
            </div>
