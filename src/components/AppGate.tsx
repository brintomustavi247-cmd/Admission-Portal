...
import { useAppSettings } from "../hooks/useAppSettings"; // ✅ নতুন ইম্পোর্ট
...
export const AppGate: React.FC<{ children: React.ReactNode }> = ({
  ...
  const { settings } = useAppSettings(); // ✅ লোকাল স্টেটের বদলে গ্লোবাল হুক
  
  const [copiedId, setCopiedId] = useState(false);

  // ❌ REMOVED: app_settings লোড করার লোকাল useEffect (এখন হুক দিয়ে হচ্ছে)
  ...
  // লোডিং স্ক্রিনে settings চেক যোগ করা হয়েছে
  if (loading || !settings) {
    return (
  ...
  if (!profile.access_enabled && profile.role !== "admin") {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#151a23] text-slate-200 p-6 text-center">
        ...
        <p className="text-sm text-slate-400 mt-2 max-w-sm">তোমার অ্যাকাউন্টের অ্যাক্সেস বর্তমানে নিষ্ক্রিয়। সাপোর্ট টিমের সাথে যোগাযোগ করো।</p>
        <div className="mt-5 rounded-2xl bg-[#0f141d] border border-white/10 px-5 py-3 flex items-center gap-3">
          <div className="text-left">
            <div className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">তোমার User ID</div>
            <code className="text-base font-black text-sky-300 tracking-widest">{uid}</code>
          </div>
          <button
            onClick={async () => {
              try {
        ...
        <p className="text-[10px] text-slate-500 mt-2 max-w-xs">এই ID-এর screenshot সাপোর্ট-এ পাঠিও — admin এক ক্লিকে অ্যাক্সেস চালু করে দিতে পারবেন</p>
        <div className="grid grid-cols-2 gap-2 mt-5 w-full max-w-xs">
          <a href={WHATSAPP} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 text-[11px] font-bold hover:bg-emerald-500/25 transition">
            <MessageCircle className="w-3.5 h-3.5" /> WhatsApp
          </a>
          <a href={TELEGRAM} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-sky-500/15 border border-sky-500/30 text-sky-300 text-[11px] font-bold hover:bg-sky-500/25 transition">
            <Send className="w-3.5 h-3.5" /> Telegram
          </a>
        </div>
        <button onClick={() => refreshProfile()} className="mt-5 px-5 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-bold cursor-pointer">আবার চেক করো</button>
      </div>
    );
  }
  ...
  // 🔥 MAIN LOGIC: settings.subscription_enabled false হলে সরাসরি children (Dashboard) রেন্ডার হবে
  if (settings.subscription_enabled && profile.role !== "admin" && !premiumActive) {
    ...
    <>
      {settings.announcement_text && (
        <div className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-[11px] font-bold px-4 py-2 text-center flex items-center justify-center gap-1.5">
          <Megaphone className="w-3.5 h-3.5 shrink-0" /> {settings.announcement_text}
        </div>
      )}
      {children}
      <FreePeriodPopup />
