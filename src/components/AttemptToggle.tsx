...
import { UserCheck, GraduationCap, AlertCircle, Compass, Zap } from 'lucide-react';
...
export const AttemptToggle: React.FC<AttemptToggleProps> = ({
  ...
  return (
    <div id="attempt-toggle-container" className="w-full max-w-2xl mx-auto my-6">
      <div className="bg-white dark:bg-[#1e2530] p-2 rounded-3xl border border-slate-200/90 dark:border-[#2a3344] shadow-sm transition-colors">
        ...
        <div className="flex items-center justify-between px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 mb-1">
          ...
          <span className="text-[11px] text-slate-400 dark:text-slate-500">
            {isSecondTimer ? '২য় বার ফিল্টার সক্রিয়' : 'সকল বিশ্ববিদ্যালয় প্রদর্শিত'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-1.5 sm:gap-2 bg-slate-100/90 dark:bg-[#232b3a]/90 p-1.5 rounded-2xl relative">
          ...
          >
            ...
            <UserCheck className={`w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 ${!isSecondTimer ? 'text-white' : 'text-slate-500 dark:text-slate-400'}`} />
            <span className="hidden sm:inline">১ম বারের পরীক্ষার্থী (1st Timer)</span>
          ...
          >
            ...
            <GraduationCap className={`w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 ${isSecondTimer ? 'text-yellow-200' : 'text-emerald-600 dark:text-emerald-400'}`} />
            <span className="hidden sm:inline">২য় বারের পরীক্ষার্থী (2nd Timer)</span>
            ...
            {isSecondTimer ? (
              <>
                ...
                <span className="text-emerald-800 dark:text-emerald-300 font-medium">
                  কার্ডে ক্লিক করে ২য় বার নীতিমালা, নম্বর কর্তন ও বিস্তারিত সময়সূচি দেখুন।
                </span>
              </>
            ) : (
              <>
                <AlertCircle className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400 shrink-0" />
                <span>কার্ডে ক্লিক করে বিস্তারিত সময়সূচি, ইউনিট ও নিয়মাবলী দেখুন।</span>
              </>
            )}
          </div>
        </div>
