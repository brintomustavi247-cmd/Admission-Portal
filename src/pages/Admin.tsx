...
import { UpdateQueueTab } from "../components/admin/UpdateQueueTab";
import {
  ArrowLeft,
  Users,
  Crown,
          ...
          <div className="relative w-full max-w-md max-h-[85vh] overflow-y-auto rounded-3xl bg-[#151b27] border border-white/10 p-5 shadow-2xl">
            ...
            <div className="space-y-2 text-[11px]">
              {[
                ...
                <div
                  key={k as string}
                  className="flex items-start justify-between gap-3 rounded-xl bg-[#0f141d] border border-white/5 px-3 py-2"
                >
                  <span className="text-slate-400 font-bold shrink-0">{k}</span>
                  <span className="text-slate-200 font-bold text-right break-all">
                    {v as string}
                  </span>
                </div>
              ))}
                ...
                <div className="space-y-2">
                  {payments
                    ...
                    .map((p) => (
                      <div
                        key={p.id}
                        className="rounded-xl bg-[#0f141d] border border-white/5 px-3 py-2"
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-black text-white">
                            ৳{toBanglaNum(p.amount)} • {p.plan}
                          </span>
                          <span
              ...
              {events.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-52 overflow-y-auto">
                  {events.map((ev) => (
                    <div
                      key={ev.id}
                      className="rounded-lg bg-[#0f141d] border border-white/5 px-3 py-1.5 text-[10px]"
                    >
                      <span className="text-sky-300 font-bold">
                        {ev.meta?.button || ev.event_type}
                      </span>
                      {ev.meta?.context && (
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              <button
                onClick={() => {
                  toggleUser(selected, "access_enabled");
                  setSelected({
                    ...selected,
                    access_enabled: !selected.access_enabled,
                  });
                }}
                className={`py-2.5 rounded-xl text-[11px] font-black cursor-pointer ${
                  selected.access_enabled
              ...
              <button
                onClick={() => {
                  toggleUser(selected, "is_premium");
                  setSelected({
                    ...selected,
